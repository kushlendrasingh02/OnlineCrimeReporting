  ...
  */
  ...
  /**
    ...
    onTranslationsFetch: function(callback) {
        // no translations defined for the form or we already have the translations
        if (typeof FormTranslation === 'undefined' ||
            FormTranslation.version === 1 ||
            FormTranslation.to ||
            JotForm.__translationsRequestAreCompleted // somehow translation request failed on card forms, and we already emptied the queue
        ) {
            callback();
            return;
        }
        ...
        var isCardForm = window.FORM_MODE === 'cardform';
        if (typeof JotForm.__onTranslationsFetchCallbackQueue === 'undefined') {
            JotForm.__onTranslationsFetchCallbackQueue = [];
            JotForm.__translationsRequestAreCompleted = false

            var responder = {
                onComplete: function (request, response) {
                    var action = request.parameters && request.parameters.action;
                    var isTranslationAction = action === 'getFormTranslations' || action === 'getCardFormTranslations';

                    if (request.method === 'get' && isTranslationAction) {
                        if (isCardForm && action === 'getFormTranslations') return; // cardforms require two requests for translations

                        // translations are fetched so, call the callbacks in order, then clean everthing up
                        JotForm.__onTranslationsFetchCallbackQueue.each(function call(cb) { cb(); });
                        delete JotForm.__onTranslationsFetchCallbackQueue;
                        JotForm.__translationsRequestAreCompleted = true
        ...
        try {
            JotForm.__onTranslationsFetchCallbackQueue.push(callback);
        } catch (e) {
            callback();
        }
    }
